...
# ==================== FUNÇÕES DE UTILIDADE (SEM PSUTIL) ====================
...
with col2:
    day_range = st.number_input("Days", value=10, min_value=1, max_value=MAX_DAYS)

total_duration_sec = min(1.2*day_range, MAX_VIDEO_DURATION)

# Criar diretório temporário seguro
temp_base = tempfile.mkdtemp()
maps_dir = os.path.join(temp_base, "maps_png")
...
if 'generate_clicked' in st.session_state and st.session_state['generate_clicked']:
    ...
    try:
        ...
        response = requests.get(
            f"https://firms.modaps.eosdis.nasa.gov/api/area/csv/{map_key}/MODIS_SP/world/{day_range}/{data_date}", 
        ...
        if not lat_col or not lon_col:
            raise ValueError("Colunas de coordenadas não encontradas")
        ...
        if not df_local.empty:
            ...
            for i in range(intro_frames):
                ...
                for spine in ax_map.spines.values():
                    spine.set_visible(False)
                png_file = os.path.join(maps_dir, f"intro_{i}.png")
                fig.savefig(png_file, facecolor='#000000', dpi=80, bbox_inches='tight', pad_inches=0.1)
                plt.close(fig)
            ...
            for i, (day, n_fires) in enumerate(fires_per_day.values):
                ...
                for k in range(n_fade_frames):
                    ...
                    png_file = os.path.join(maps_dir, f"map_{i}_{k}.png")
                    fig.savefig(png_file, facecolor='#000000', dpi=80, bbox_inches='tight', pad_inches=0.05)
                    plt.close(fig)
                    img = Image.open(png_file).convert("RGB")
                    img = img.resize((TARGET_WIDTH, TARGET_HEIGHT), Image.Resampling.LANCZOS)
                    img.save(png_file, quality=85, optimize=True)
            ...
            clip.write_videofile("fires_video.mp4", codec="libx264", audio_codec="aac", 
                               verbose=False, logger=None, threads=2, bitrate="2000k")
            ...
            logging.info(f"Video generated: {n_days} days, {len(images_files)} frames")
        ...
        progress_placeholder.empty()
        status_placeholder.empty()
        st.error("⏱️ Timeout ao buscar dados. Tente novamente.")
        st.session_state['generate_clicked'] = False
...
<div style='text-align: center; color: #666; font-size: 12px;'>
    🔥 Hear the Fire v2.0 | NASA FIRMS Data
</div>
""", unsafe_allow_html=True)
